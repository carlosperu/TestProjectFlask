...
class SMWinservice(win32serviceutil.ServiceFramework):
    ...
    def callback(ch, method, properties, body):
        ...
        print(" File Name = {}".format(data['FileName']))
        logging.info(' Reading file called : {} '.format(data['FileName']))
    ...
    def main(self):
        ...
        connection = pika.BlockingConnection(pika.ConnectionParameters(host=ipBroker, credentials=credentials))

        channel = connection.channel()

        FORMAT = '%(asctime)-15s  %(levelname)s  ::::  %(message)s'
        logging.basicConfig(filename="TestLog.log", level=logging.INFO, format=FORMAT)

        channel.basic_consume(self.callback, queue='LogQueue', no_ack=True)
